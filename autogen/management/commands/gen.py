...
OUT_PATH =  os.path.join(FRONTEND_DIR, 'src','app','viewsactions')
...
def create_dirs(conf):
    root_path = '%s/src/app/viewsactions/%s' % (FRONTEND_DIR, conf['root'])
    # out_path = '%s/viewsactions/%s' % (root_path, conf['dirname'])
    # store_path = '%s/viewsactions/%s/store' % (FRONTEND_DIR, conf['dirname'])
...
def gen_tabs(conf):
    try:
        for tab in conf["tabs"]:
            ...
            'viewsactions', conf['root'],conf['dirname'],'view','tabs')
            ...
            for i in file_tab_list:
                ...
                'viewsactions', conf['root'],conf['dirname'],filename)
                print(filename)
            ...
            out_path = os.path.join(FRONTEND_DIR, 'src','app',\
                'viewsactions', conf['root'],conf['dirname'],file_list[13].replace('tpl', tab['selector']))
            out = template.render(conf)
            print(out_path)
